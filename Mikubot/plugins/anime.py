...
from Mikubot import BOT_USERNAME, MESSAGE_DUMP, MONGO_DB_URI, app
from Mikubot.utils.custom_filters import PREFIX_HANDLER
...
_DATABASE: AgnosticDatabase = _MGCLIENT["MikubotAnime"]
    ...
    async def wrapper(_, message: Message):
            ...
            try:
                ...
                if nut - out < 1.2:
                    ...
                    if USER_WC[user] == 3:
                        ...
                        await clog("Mikubot", f"UserID: {user}", "SPAM")
                    if USER_WC[user] == 5:
                        ...
                        await clog("Mikubot", f"UserID: {user}", "BAN")
                        return
        ...
        try:
            ...
            try:
                ...
                await clog("Mikubot", e, "FAILURE", msg=message)
        ...
        if user in BOT_OWNER or user == int(cqowner):
            ...
            try:
                ...
                try:
                    ...
                    await clog("Mikubot", e, "FAILURE", cq=c_q)
        else:
            if cqowner_is_ch:
                if user_valid:
                    try:
...
example: /anime Sword Art Online"""
    ...
    try:
        ...
        await clog("Mikubot", title_img, "LINK", msg=message)
...
example: /manga Sword Art Online"""
    ...
    try:
        ...
        await clog("Mikubot", pic, "LINK", msg=message)
        await client.send_photo(gid, failed_pic, caption=finals_, reply_markup=buttons)
...
async def character_cmd(client: Client, message: Message, mdata: dict):
    ...
    try:
        ...
        await clog("Mikubot", img, "LINK", msg=message)
...
example: /airing Sword Art Online"""
    ...
    try:
        ...
        await clog("Mikubot", coverImg, "LINK", msg=message)
...
This material is marked 18+ and not allowed in this group""",
    ...
    try:
        ...
        await clog("Mikubot", pic, "LINK", msg=cq)
        await cq.edit_message_media(
...
➠ *Mikubot provides you the best anime-based commands including anime news and much more!*
